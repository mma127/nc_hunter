...
const TooltipContent = ({x, y, playerNames, npcNames, planeData}) => {
  ...
  return (
    <>
      ...
      {playerNames.map(name => <Box key={`player-${name}`}><Typography variant="body" className={classes.player}>{name}</Typography></Box>)}
      {npcNames.map(name => <Box key={`npc-${name}`}><Typography variant="body" className={classes.npc}>{name}</Typography></Box>)}
    </>
  )
}
...
export const Tile = ({x, y, handleClick}) => {
  ...
  if (!isBlank) {
    ...
    const playerNames = tile.players.names;
    const npcNames = tile.npcs.names;
    const haveNames = playerNames.length > 0 || npcNames.length > 0;
      ...
      <Tooltip key={`tooltip-${x}-${y}`}
        ...
        >
          <CardActionArea className={classes.cardActionArea} onClick={onClick}>
            ...
            {haveNames ? <Names playerNames={playerNames} npcNames={npcNames}/> : null}
          </CardActionArea>
        </Card>
      </Tooltip>
    )
